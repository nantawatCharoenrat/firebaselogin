import 'package:firebaselogin/showdetailcat2.dart';
import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

class Catpage2 extends StatelessWidget {
  final store = FirebaseFirestore.instance;
  Catpage2({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: store.collection('cat2').snapshots(),
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        return Scaffold(
          appBar: AppBar(
            title: const Text('แมวสีสวาด'),
  ...
  IconButton buildAddButton(context) {
    return IconButton(
        ...
        onPressed: () {
          ...
          Navigator.pushNamed(context, '/addcat2');
        });
  }
        ...
        return ListTile(
          ...
          onTap: () {
            ...
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => CatDetail2(model['title'])));
          },
        );
      },
    );
